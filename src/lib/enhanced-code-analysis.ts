...
export interface SemanticChange {
  type: 'api-addition' | 'api-removal' | 'api-modification' | 'behavior-change' | 'performance-change';
...
export async function performEnhancedCodeAnalysis(
  ...
  const migrationComplexity = assessMigrationComplexity(semanticChanges, apiChanges, breakingPatterns);
  const riskAssessment = assessCodeRisk(semanticChanges, apiChanges, breakingPatterns, fileImpactAnalysis);
...
function analyzeLineForSemanticChanges(line: string, file: string, lineNumber: number): SemanticChange | null {
  ...
  // Export changes
  if (content.includes('export') && (content.includes('function') || content.includes('class') || content.includes('const'))) {
    return {
...
function parseApiChange(line: string, file: string, lineNumber: number): ApiChange | null {
  ...
  const functionMatch = content.match(/(export\s+)?(function\s+|const\s+)(\w+)(\s*=\s*|\s*)\(([^)]*)\)/);
  if (functionMatch) {
    const [, exportKeyword, declaration, name, assignment, params] = functionMatch;
...
function extractFilesFromMatches(matches: string[]): string[] {
  // Extract unique file names from diff content
  // This is a simplified implementation
  return ['multiple files']; // Would implement proper file extraction
}

function generateSuggestedFix(pattern: RegExp, matches: string[]): string | undefined {
...
async function analyzeFileImpact(codeDiff: CodeDiff): Promise<FileImpactAnalysis> {
...
function generateRecommendations(risk: string, riskFactors: RiskFactor[]): string[] {
