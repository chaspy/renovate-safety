...
export function generateActionableRecommendations(
  ...
  if (result.breakingChanges.length > 0) {
    recommendations.push(...getBreakingChangeRecommendations(result.breakingChanges, result.apiUsages));
  }
...
function getCriticalRiskRecommendations(result: AnalysisResult): ActionableRecommendation[] {
  return [{
...
function getSafeRecommendations(result: AnalysisResult): ActionableRecommendation[] {
  return [{
...
function getBreakingChangeRecommendations(
  ...
  if (removals.length > 0) {
    recommendations.push({
      ...
      automatable: false,
      resources: [
        'Link to package migration guide',
        'Search for migration codemods'
      ]
    });
  }
...
function isFrontendPackage(packageName: string): boolean {
  ...
  return frontendIndicators.some(indicator => 
    packageName.toLowerCase().includes(indicator)
  );
}
